...
<div class="card">
    <div class="card-header">
        {{ trans('global.create') }} {{ trans('cruds.masterFormPayment.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.master-form-payments.store") }}" enctype="multipart/form-data">
            @csrf
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('buyer') ? 'is-invalid' : '' }}" name="buyer_id" id="buyer_id">
                    @foreach($buyers as $id => $entry)
                        <option value="{{ $id }}" {{ old('buyer_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
                @if($errors->has('buyer'))
                    <div class="invalid-feedback">
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('conference') ? 'is-invalid' : '' }}" name="conference_id" id="conference_id">
                    @foreach($conferences as $id => $entry)
                        <option value="{{ $id }}" {{ old('conference_id') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>

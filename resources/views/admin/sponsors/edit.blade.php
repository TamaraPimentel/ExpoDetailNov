...
<div class="card">
    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.sponsors.update", [$sponsor->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <label for="web_url">{{ trans('cruds.sponsor.fields.web_url') }}</label>
                <input class="form-control {{ $errors->has('web_url') ? 'is-invalid' : '' }}" type="text" name="web_url" id="web_url" value="{{ old('web_url', $sponsor->web_url) }}">
                @if($errors->has('web_url'))
                    <div class="invalid-feedback">
                        {{ $errors->first('web_url') }}
                    </div>
                @endif
                <span class="help-block">{{ trans('cruds.sponsor.fields.web_url_helper') }}</span>
            </div>
            <div class="form-group">
